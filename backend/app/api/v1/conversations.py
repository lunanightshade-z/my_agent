...
from app.dependencies import get_conversation_repository, get_chat_service
...
def create_conversation(
    ...
    """创建新的对话会话"""
    try:
        db_conversation = repo.create(title=conversation.title)
...
def get_conversations(
    ...
    """获取所有对话会话列表（按更新时间倒序）"""
    try:
        conversations = repo.get_all(skip=skip, limit=limit)
...
def update_conversation_title(
    ...
    """更新会话标题"""
    conversation = repo.update_title(conversation_id, request.title)
    if not conversation:
        raise HTTPException(status_code=404, detail="会话不存在")
...
async def generate_title(
    ...
    """基于第一条消息自动生成会话标题"""
    try:
        title = await chat_service.generate_title(conversation_id, request.first_message)
        return SuccessResponse(
            success=True,
            data={"title": title}
        )
